...
##### JVM七种垃圾收集器
Serial收集器：最基本的，它在进行垃圾收集时，必须暂停所有的工作线程，直到结束
ParNew收集器：Serial收集器的多线程版本
Parallel Scavenge：吞吐量（运行用户代码时间/(运行用户代码时间+垃圾收集时间)）优先收集器，使用复制算法，并行的多线程收集器
Serial Old:Serial收集器的老年代版本
Parallel Old:Parallel Scavenge收集器的老年代版本。

CMS(Concurrent Mark Sweep):是一种以获取最短停顿时间为目标的收集器，整个过程可以分为：
   初始标记:标记GCRoots能够到达的对象，速度很快，会产生"Stop the world"
   并发标记:进行GCRoot Tracing的过程
   重新标记:为了修正并发标记期间，因用户程序继续运行而呆滞标记变动的那一部分对象的标记,会产生"Stop the world",但是时间比并发标记短。
   并发清除
   
   优点：并发收集、低停顿
   缺点：对CPU敏感，由于使用"标记-清除算法"所以会导致浮动垃圾。

G1(Garbage-First):目前最前沿的成果，是一种回收价值优先的收集器，将整个JVM堆分成多个大小相等的独立区域，G1跟踪各个Region里面垃圾堆积的价值大小，优先回收价值最大的Region。
   回收过程分为：
      初始标记:
      并发标记:进行可达性分析，找出存活对象
      最终标记:将Remembered Set Logs的数据合并到Remembered Set中
      筛选回收：对各个Region的回收价值和成本进行排序，根据用户所希望的GC停顿时间来执行回收计划。
    优点：并行、并发，GI整体上看使用"标记-整理"算法，从局部上看，每个"region"是使用"复制"算法实现，可预测的停顿。
    
总体而言，可以无条件选择G1，除非你的应用追求吞吐量。
